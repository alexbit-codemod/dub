    ...
    <div className="flex flex-col gap-6 rounded-lg border border-neutral-200 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-content-subtle text-sm font-normal leading-5">{t('descriptions.default-links-purpose')}</p>
        </div>
...
const t = useTranslations("group-default-links");
  ...
  const { setShowConfirmModal, confirmModal } = useConfirmModal({
    ...
    description: (
      ...
      component0: (chunks) => <strong>{getPrettyUrl(getUrlWithoutUTMParams(link.url))}</strong>
    })}
        <br />
        <br />{t('warnings.delete-consequences')}<br />
        <br />{t('suggestions.edit-instead')}</>
    ),
    confirmText: "Delete",
    onConfirm,
  });
  ...
  return (
    <div className="flex h-[200px] flex-col items-center justify-center gap-6 rounded-lg bg-neutral-50 p-4">
      ...
      <div className="flex flex-col gap-1 text-center">
        <h2 className="text-content-emphasis text-base font-medium">{t('headings.empty-state')}</h2>
        <p className="text-content-subtle text-sm">{t('messages.no-links-created')}</p>
      </div>
    </div>
  );
}
